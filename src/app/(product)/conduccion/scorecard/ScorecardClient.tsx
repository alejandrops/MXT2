...
export function ScorecardClient({ data }: Props) {
  ...
  const columns: ColumnDef<RankedDriver & { _idx: number }>[] = [
    ...
    },
    {
      ...
      sortable: false,
      render: (r) => {
        // anchorIso = "YYYY-MM-DD" · derivamos "YYYY-MM" y "YYYY"
        const monthly = data.anchorIso.slice(0, 7);
        const annual = data.anchorIso.slice(0, 4);
        return (
          <span className={styles.boletinActions}>
            <a
              href={`/conduccion/boletin/conductor/${r.personId}/${monthly}`}
              target="_blank"
              rel="noopener noreferrer"
              className={styles.boletinLink}
              title={`Boletín mensual · ${monthly}`}
              onClick={(e) => e.stopPropagation()}
            >
              M
            </a>
            <a
              href={`/conduccion/boletin/conductor/${r.personId}/${annual}`}
              target="_blank"
              rel="noopener noreferrer"
              className={styles.boletinLink}
              title={`Boletín anual · ${annual}`}
              onClick={(e) => e.stopPropagation()}
            >
              A
            </a>
