...
import type { GroupBoletinData } from "@/lib/queries/group-boletin-data";
import styles from "./Boletin.module.css";

// ═══════════════════════════════════════════════════════════════
//  GroupBoletin · S5-E2 · Tufte + B&N first
//  ─────────────────────────────────────────────────────────────
//  Boletín imprimible de grupo · 4 secciones:
//    01 · Score grupal + KPIs + comparativa
//    02 · Distribución de infracciones agregadas
//    03 · Ranking de conductores · top 5 + bottom 5
//    04 · Scatter (km vs score) · panorama del grupo
//    05 · Evolución temporal (4 semanas o 12 meses)
//
//  Score 42px · misma decisión visual que el boletín de conductor.
// ═══════════════════════════════════════════════════════════════
      ...
      <div className={styles.page}>
        ...
        <div className={styles.titleBlock}>
          <h1 className={styles.title}>
            {period.kind === "monthly"
              ? `Boletín mensual del grupo ${data.group.name}`
              : `Boletín anual del grupo ${data.group.name}`}
          </h1>
          <div className={styles.subtitle}>
            <strong>{period.label}</strong> · {data.group.accountName}
            {data.group.parentName && <> · sub-grupo de {data.group.parentName}</>}
            {" · "}
            {data.group.totalAssets} {data.group.totalAssets === 1 ? "vehículo asignado" : "vehículos asignados"}
          </div>
        </div>

        <div className={styles.lead}>{lead}</div>

        {/* ── 01 · Score grupal + KPIs ───────────────────── */}
        <SectionHeader num="01" title={period.kind === "monthly" ? "Calificación grupal y volumen" : "Calificación grupal y volumen anual"} />

        <ScoreLine
          score={data.summary.score}
          prevScore={data.prev.score}
          period={period}
          kpis={getGroupKpis(data, period)}
        />

        {/* ── 02 · Infracciones del grupo ────────────────── */}
        <SectionHeader
          num="02"
          title={period.kind === "monthly" ? "Infracciones del grupo" : "Infracciones del año"}
        ...
        {/* ── 03 · Ranking de conductores ────────────────── */}
        {data.summary.activeDrivers > 0 && (
          <>
            <SectionHeader
              num="03"
              title="Ranking de conductores"
              summary={`${data.summary.activeDrivers} ${data.summary.activeDrivers === 1 ? "conductor activo" : "conductores activos"}`}
            />
            ...
            <div className={styles.rankingGrid}>
              <div>
                ...
                <RankingTable rows={data.topDrivers} />
              </div>
              {data.bottomDrivers.length > 0 && (
                <div>
                  ...
                  <RankingTable rows={data.bottomDrivers} dim />
                </div>
              )}
            </div>
          </>
        ...
        {data.scatter.length > 0 && (
          <>
            <SectionHeader
              num="04"
              title="Panorama · distancia vs score"
              summary="cada punto es un conductor del grupo"
            />
...
function getGroupKpis(
  ...
  const driversTrend = makeTrendText(
    data.summary.activeDrivers,
    data.prev.activeDrivers,
    period,
    "abs",
  );
  ...
  return [
    ...
    { label: "En ruta", value: String(horas), unit: "h" },
    {
      label: "Conductores",
      value: String(data.summary.activeDrivers),
      trend: driversTrend || "activos en el período",
    },
  ];
}

// ═══════════════════════════════════════════════════════════════
//  Distribution stack · idéntico al driver
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
//  Ranking table · top/bottom drivers
// ═══════════════════════════════════════════════════════════════
...
function RankingTable({
  ...
  rows: GroupBoletinData["topDrivers"];
  dim?: boolean;
}) {
  return (
...
// ═══════════════════════════════════════════════════════════════
//  Scatter chart · puntos (km, score)
// ═══════════════════════════════════════════════════════════════

function ScatterChart({
  points,
}: {
  points: GroupBoletinData["scatter"];
}) {
  if (points.length === 0) return null;

  // viewBox 0 0 600 200 · margen 40 izq · 30 der · 10 sup · 30 inf
  const xMin = 40;
  const xMax = 570;
  const yMin = 10;
  const yMax = 170;
  ...
  // X · 0 a maxKm (con padding)
  const xFor = (km: number) => xMin + (km / maxKm) * (xMax - xMin);
  // Y · 0 (bottom) a 100 (top)
  const yFor = (score: number) =>
    yMax - (Math.max(0, Math.min(100, score)) / 100) * (yMax - yMin);

  // Líneas guía horizontales · zona verde 80 · zona amarilla 60
  const y80 = yFor(80);
  const y60 = yFor(60);
  ...
  return (
    ...
    >
      ...
      >
        DISTANCIA RECORRIDA EN EL PERÍODO
      </text>

      {/* Puntos · ○ verde / ● amarilla o roja */}
      {points.map((p) => {
        ...
        return (
          ...
          >
            <title>
              {p.name} · {p.score}/100 · {formatKm(p.distanceKm)} km · {p.infractionCount} inf.
            </title>
          </circle>
        );
      })}
...
// ═══════════════════════════════════════════════════════════════
//  Evolution sections · idénticas al driver
// ═══════════════════════════════════════════════════════════════

function MonthlyEvolutionSection({ data }: { data: GroupBoletinData }) {
  ...
  return (
    <>
      <SectionHeader num="05" title="Evolución del grupo · semanas del mes" />
...
function AnnualEvolutionSection({ data }: { data: GroupBoletinData }) {
